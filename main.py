...
APPROVAL_FILE_URL = "https://raw.githubusercontent.com/rihitoo/spam-share/main/approval.txt"
LOCAL_KEY_PATH = os.path.join("Rikuo", "approval.txt")
...
def check_approval():
    ...
    try:
        response = requests.get(APPROVAL_FILE_URL)
        response.raise_for_status()
        valid_keys = response.text.strip().splitlines()
    except Exception as e:
        console.print(Panel(f"❌ Unable to fetch approval list:\n{e}", style="red"))
        exit()

    if saved_key not in valid_keys:
        console.print(Panel("🔐 [red]Approval key required to access this tool.[/red]"))
        user_key = console.input("[bold cyan]Enter your approval key:[/bold cyan] ").strip()
        if user_key in valid_keys:
            with open(LOCAL_KEY_PATH, "w") as f:
                f.write(user_key)
            console.print("[bold green]✅ Approved! Access granted.")
            time.sleep(1)
        else:
            console.print("[bold red]❌ Invalid approval key. Access denied.[/bold red]")
            exit()
...
def spam_share():
    ...
    link = console.input("[bold green]Enter the post link to share: ").strip(); linex()

    try:
        total_shares = int(console.input("[bold green]Enter total number of shares: [/]").strip()); linex()
        if total_shares <= 0:
    ...
    try:
        delay = float(console.input("[bold green]Enter delay between shares (in seconds): ").strip()); linex()
        if delay < 0:
...
def main():
    check_approval()  # Approval check added
    banner()
